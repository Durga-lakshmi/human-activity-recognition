...
class PostureSitStandFeatures(nn.Module):
    """
    Input: x_raw [B, T, 6] (ax, ay, az, gx, gy, gz)
    Output: f_posture [B, 1 + 3 + 3] = [tilt_angle, g_unit(3), acc_var(3)]
    """
    ...
    def forward(self, x_raw: torch.Tensor) -> torch.Tensor:
        ...
        # 1) Estimated gravitational vector g ≈ time-averaged acceleration
        g = acc.mean(dim=1)               # [B, 3]
        g_norm = torch.norm(g, dim=1, keepdim=True) + self.eps
        g_unit = g / g_norm               # [B, 3]

        # 2) Trunk inclination angle: Angle relative to the “vertical axis” (using the z-component)
        cos_theta = torch.clamp(g_unit[:, 2], -1.0 + 1e-6, 1.0 - 1e-6)
        tilt_angle = torch.acos(cos_theta)    # [B]
        tilt_angle = tilt_angle.unsqueeze(1)  # [B, 1]

        # 3) Degree of Stasis: Acceleration Variance
        acc_var = acc.var(dim=1)          # [B, 3]

        # concatenate into posture features
        f_posture = torch.cat([tilt_angle, g_unit, acc_var], dim=1)  # [B, 1+3+3]
        return f_posture